...
- **v2.12.0确认**: 代码层面迁移100%完成，src/目录无#635BFF残留，所有CSS变量和组件已统一使用#5E6AD2
...
## 🏷️ Token 命名系统 v2.12 (Token Naming System)

### 设计理念

超级洞察使用**语义化 Token 命名**体系，让设计系统更易理解和维护。从 v2.2.0 开始，我们统一了 Token 命名规范，并在 v2.12.0 增强了废弃警告和迁移指南。

**v2.12.0 更新**:
- ✅ 增强 deprecation 警告（包含详细迁移映射和时间表）
- ✅ 更新废弃时间表：v2.14.0 将移除所有遗留 Token
- ✅ 添加防 FOUC 主题初始化脚本
- ✅ 统一主题管理到 UIStore（废弃 ThemeContext）
...
### 旧版 Token (v2.14.0 将移除)

**⚠️  以下 Token 已标记为废弃（v2.2.0），将在 v2.14.0 移除，请在新代码中避免使用:**
...
- **v2.2.0**: 语义化 Token 引入，旧 Token 标记为 deprecated
- **v2.12.0 (当前)**: 增强 deprecation 警告，添加详细迁移指南
- **v2.13.0 (计划)**: 添加 stylelint 规则，新代码使用旧 Token 将触发警告
- **v2.14.0 (breaking)**: 移除所有旧 Token，必须使用语义化 Token
- **过渡期**: v2.12.0 → v2.14.0（2个版本周期，预计3-6个月）

**建议策略**:
1. 新代码一律使用语义化 Token
2. 现有代码渐进式迁移（非强制）
3. v2.13.0 后新 PR 不得包含旧 Token
4. v2.14.0 前完成全部迁移
