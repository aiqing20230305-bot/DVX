...
export async function exportInsightsToPDF(
  ...
  // 添加封面页
  addCoverPage(doc, opts, colors)

  // 添加新页（内容页）
  doc.addPage()

  // 页眉
  addHeader(doc, opts.title, colors, opts.fontSize)
  ...
  autoTable(doc, {
    ...
    didDrawPage: (data) => {
      ...
      addFooter(doc, currentPage, pageCount, colors, opts.fontSize, opts.showPageNumbers)
    }
  })
