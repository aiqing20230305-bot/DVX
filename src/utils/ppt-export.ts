 ...
 * 默认导出选项
 */
const DEFAULT_OPTIONS: Required<PPTExportOptions> = {
  title: '数据报告',
  aspectRatio: '16:9',
...
export async function exportInsightsToPPT(
  ...
  // 合并默认选项
  const opts: Required<PPTExportOptions> = { ...DEFAULT_OPTIONS, ...options }
  ...
  // 1. 封面页
  createCoverSlide(pres, opts)

  // 2. 摘要页
  createSummarySlide(pres, insights.length, '洞察', opts)
...
export async function exportTopicsToPPT(
  ...
  // 2. 摘要页
  createSummarySlide(pres, topics.length, '选题', opts)
