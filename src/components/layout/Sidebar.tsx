  ...
  return (
    <>
      <aside className={[
            ...
            <div className="relative">
              <div className="flex gap-1">
                <button
                  ...
                  className="flex-1 min-w-0 flex items-center gap-2 px-3 py-2 rounded-l-lg bg-slate-800 border border-slate-700 hover:border-indigo-600 transition-colors text-left"
                  title="查看项目详情"
                >
                  <div className="min-w-0 flex-1">
                    <div className="text-xs text-slate-500 mb-0.5">当前项目</div>
                ...
                <button
                  ...
                  className="px-2 rounded-r-lg bg-slate-800 border border-l-0 border-slate-700 hover:border-slate-600 transition-colors flex items-center"
                >
                  <ChevronDown size={14} className={`text-slate-400 transition-transform ${projectDropdown ? 'rotate-180' : ''}`} />
                </button>
              </div>
                ...
                <div className="absolute top-full left-0 right-0 mt-1 bg-slate-800 border border-slate-700 rounded-lg shadow-xl shadow-black/50 z-50 overflow-hidden animate-fade-in-down">
                  {projects.map((project, index) => (
                    ...
                    >
                      ...
                      {activeProjectId === project.id && <Check size={14} className="text-indigo-400 flex-shrink-0" />}
                      <button
                        className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-red-900/30 hover:text-red-400 text-slate-500 transition-all"
                  ...
                  <div
                    className="flex items-center gap-2 px-3 py-2.5 hover:bg-slate-700 cursor-pointer border-t border-slate-700 text-indigo-400"
        ...
        <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
          {navItems.map(({ to, label, icon: Icon, end, divider }) => (
            <React.Fragment key={to}>
              {divider && <div className="h-px bg-slate-700 my-2" />}
              <NavLink
                to={to}
                end={end}
                className={({ isActive }) => [
        ...
        {/* Bottom controls */}
        <div className="px-2 py-3 border-t border-slate-800 flex items-center gap-1">
          <button
            onClick={toggleTheme}
          ...
          {!sidebarCollapsed && (
            <button
              ...
              className="flex items-center gap-2 flex-1 px-2 py-2 rounded-lg text-slate-400 hover:text-slate-200 hover:bg-slate-800 transition-colors text-sm"
            >
              <Plus size={15} />
              新建项目
            </button>
          ...
          <button
            ...
            className="flex items-center justify-center w-9 h-9 rounded-lg text-slate-400 hover:text-slate-200 hover:bg-slate-800 transition-colors ml-auto"
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 text-slate-200 placeholder-slate-500 text-sm resize-none"
              />
            </div>
          </div>
        )}
