    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-[#646A73] mt-1">触发历史</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-[#F7F8FA] border border-[#DEE0E3] rounded-lg p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="text-[#646A73] text-sm">总触发次数</h3>
            </div>
            <p className="text-2xl font-bold text-[#1F2329]">{triggerStats.total_triggers}</p>
          </div>

          <div className="bg-[#F7F8FA] border border-[#DEE0E3] rounded-lg p-4">
            <div className="flex items-center gap-2 mb-2">
              <CheckCircle size={18} className="text-green-400" />
              <h3 className="text-[#646A73] text-sm">已弹出次数</h3>
            </div>
            <p className="text-2xl font-bold text-[#1F2329]">{triggerStats.shown_count}</p>
          </div>

          <div className="bg-[#F7F8FA] border border-[#DEE0E3] rounded-lg p-4">
            <div className="flex items-center gap-2 mb-2">
              <CheckCircle size={18} className="text-[#5B8EFF]" />
              <h3 className="text-[#646A73] text-sm">已回答次数</h3>
            </div>
            <p className="text-2xl font-bold text-[#1F2329]">{triggerStats.answered_count}</p>
          </div>

          <div className="bg-[#F7F8FA] border border-[#DEE0E3] rounded-lg p-4">
            <div className="flex items-center gap-2 mb-2">
              <Activity size={18} className="text-yellow-400" />
              <h3 className="text-[#646A73] text-sm">回答完成率</h3>
            </div>
            <p className="text-2xl font-bold text-[#1F2329]">{triggerStats.answer_rate}%</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-[#F7F8FA] border border-[#DEE0E3] rounded-lg p-6">
        <h2 className="text-lg font-semibold text-[#1F2329] mb-4">触发记录</h2>

        {triggers.length === 0 ? (
          <div className="text-center py-8 text-[#8F959E]">暂无触发记录</div>
        ) : (
          <div className="space-y-3">
            {triggers.map((trigger) => (
              <div
                key={trigger.id}
                className="flex items-center gap-4 p-4 bg-[#F2F3F5]/50 rounded-lg"
              >
                {/* Status Icon */}
                <div>
                  {trigger.answered === 1 ? (
                    ...
                    <XCircle size={24} className="text-[#C9CDD4]" />
                  )}
                </div>

                {/* Content */}
                <div className="flex-1">
                  <div className="text-[#1F2329] font-medium mb-1">
                ...
                <div>
                  {trigger.answered === 1 ? (
                    ...
                    <span className="px-2 py-1 bg-[#DEE0E3] text-[#8F959E] text-xs rounded">
                      未弹出
                    </span>
                  )}
                </div>
